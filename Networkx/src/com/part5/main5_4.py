...
if __name__ == '__main__':
    ...
    for sheetObj in sheet_list:
        ...
        for row_index in range(1, sheet_nrow-1):
            ...
            level = int(sheet.cell(row_index, 2).value)
    ...
    for sheetObj in sheet_list:
        ...
        for row_index in range(1, sheet_nrow-1):
            ...
            term = sheet.cell(row_index, 4).value
            tenderer = sheet.cell(row_index, 5).value
            level = int(sheet.cell(row_index, 8).value)

            if tenderer in both_users:
                G1.add_node(tenderer, {'color': 'blue'})
                G1.add_node(level, {'color': 'red'})
                G1_level_nodes.append(level)
                G1.add_edge(tenderer, level)
            else:
                G2.add_node(tenderer, {'color': 'blue'})
                G2.add_node(level, {'color': 'red'})
                G2.add_edge(tenderer, level)
                G2_level_nodes.append(level)
    ...
    G1_level = nx.project(G1, set(G1_level_nodes))
    # G1_project_level = bipartite.projected_graph(G1, G1_level)
    # G1_project_user = bipartite.projected_graph(G1, G1_user)
    G1_level_degree_dict = G1_level.degree()
    G1_y1 = []
    G1_x1 = []
    for G1_key in G1_level_degree_dict.keys():
        G1_x1.append(G1_key)
        G1_y1.append(G1_level_degree_dict[G1_key])
    print(G1_x1)
    print(G1_y1)
    print(G1_level_degree_dict)
    ...
    # G1_NSet = bipartite.sets(G1)
    # G1_user = nx.project(G1, set(tenderer_nodes))
    G2_level = nx.project(G2, set(G2_level_nodes))
    # G1_project_level = bipartite.projected_graph(G1, G1_level)
    # G1_project_user = bipartite.projected_graph(G1, G1_user)
    G2_level_degree_dict = G2_level.degree()
    G2_y1 = []
    G2_x1 = []
    for G2_key in G2_level_degree_dict.keys():
        G2_x1.append(G2_key)
        G2_y1.append(G2_level_degree_dict[G2_key])
    print(G2_x1)
    print(G2_y1)
    print(G2_level_degree_dict)
