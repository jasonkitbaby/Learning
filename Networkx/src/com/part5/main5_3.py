...
if __name__ == '__main__':
    ...
    for sheetObj in sheet_list:
        ...
        for row_index in range(1, sheet_nrow-1):
            ...
            term = int(sheet.cell(row_index, 2).value)
    ...
    for sheetObj in sheet_list:
        ...
        for row_index in range(1, sheet_nrow-1):
            ...
            if tenderer in both_users:
                ...
                G1.add_node(rate, {'color': 'red'})
                G1_rate_nodes.append(rate)
                G1.add_edge(tenderer, rate)
            else:
                G2.add_node(tenderer, {'color': 'blue'})
                G2.add_node(rate, {'color': 'red'})
                G2.add_edge(tenderer, rate)
                G2_rate_nodes.append(rate)
    ...
    G1_rate = nx.project(G1, set(G1_rate_nodes))
    # G1_project_term = bipartite.projected_graph(G1, G1_term)
    # G1_project_user = bipartite.projected_graph(G1, G1_user)
    G1_rate_degree_dict = G1_rate.degree()
    G1_y1 = []
    G1_x1 = []
    for G1_key in G1_rate_degree_dict.keys():
        G1_x1.append(G1_key)
        G1_y1.append(G1_rate_degree_dict[G1_key])
    print(G1_x1)
    print(G1_y1)
    print(G1_rate_degree_dict)
    ...
    # G1_NSet = bipartite.sets(G1)
    # G1_user = nx.project(G1, set(tenderer_nodes))
    G2_rate = nx.project(G2, set(G2_rate_nodes))
    # G1_project_term = bipartite.projected_graph(G1, G1_term)
    # G1_project_user = bipartite.projected_graph(G1, G1_user)
    G2_rate_degree_dict = G2_rate.degree()
    G2_y1 = []
    G2_x1 = []
    for G2_key in G2_rate_degree_dict.keys():
        G2_x1.append(G2_key)
        G2_y1.append(G2_rate_degree_dict[G2_key])
    print(G2_x1)
    print(G2_y1)
